...
/**
 *	 클라이언트(안드로이드)에서 넘겨진
 *	수신값에 따라 마우스 이벤트 및 키보드 이벤트를 수행한다.
 *	이벤트 수행은 Robot 객체를 이용하여 처리한다.
 */
public class AutoBot {
			...
			if(tokens[0].equals("CLICK")){
				...
				if(special_key){
					...
					switch(key_code){
						...
						case 3 : key = KeyEvent.VK_HOME; break;
						case 135: key = KeyEvent.VK_F5; break;
						default: key = -1; break;
					}
					keyBoardPress(key);
					special_key = false;
	...
	public void type(char character) {
        ...
        case '←': keyBoardPress(KeyEvent.VK_LEFT); break;
        case '→': keyBoardPress(KeyEvent.VK_RIGHT); break;
	...
	public void type2(String str) {
		if(str.equals("home"))
			keyBoardPress(KeyEvent.VK_HOME);
		else if(str.equals("f5"))
			keyBoardPress(KeyEvent.VK_F5);
		else if(str.equals("shiftf5"))
			keyBoardPress(KeyEvent.VK_SHIFT, KeyEvent.VK_F5);
		else if(str.equals("end"))
			keyBoardPress(KeyEvent.VK_END);
